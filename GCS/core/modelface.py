...
#            self.model_frames[cnt] = (list(roi_frames['framenumber'])[cnt], self.model_frames[cnt][1],self.model_frames[cnt][2])
    ...
    def apply_model_on_frames(self, *args, **kwargs):
        ...
        with Progress(activity, len(self.model_frames)) as pbar:
            ...
            partition = 5
            for part in range(0, partition):
                self.model_frames_part = self.model_frames[part * len(self.model_frames):]
                self.model_frames_part = utils.get_images(self.outputfilenamepath, 
                                                          self.model_frames)   
        ...
        for tup in range(len(self.model_frames)):
            frame, group, result = self.model_frames[tup]
            if result != -1:
                model_success += 1

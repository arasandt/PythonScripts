...
noise =      "*##*###*###*****##*##****#*##*###*#****##******##*#**#*#**######"
schematas = ("1##1###1###11111##1##1111#1##1###1#1111##111111##1#11#1#11######", # 64 attributes
...
def main(extended=True, verbose=True):
    ...
    # we create a target set on length 30 using the schemata. Schemata says which value should be 1 or 0. # means any random value.
    
    species = [toolbox.species() for _ in range(NUM_SPECIES)] # 1 species contain 50 individuals. each indvidual contains 64 attributes.
    species_index = list(range(NUM_SPECIES))
    last_index_added = species_index[-1]
    # Init with random a representative for each species
    representatives = [random.choice(species[i]) for i in range(NUM_SPECIES)] # choose a random represtative from each specieis
    best_fitness_history = [None] * IMPROVMENT_LENGTH # keep track of fitness history
    ...
    while g < ngen:
        ...
        for (i, s), j in zip(enumerate(species), species_index):
            ...
            s = algorithms.varAnd(s, toolbox, 0.6, 1.0) # crossover and mutate (cxpb and mutpb) all individuals

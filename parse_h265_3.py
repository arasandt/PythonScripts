...
class video_parameter_set_rbsp(object):
    def __init__(self, s):
        ...
        print('position before flag',s.pos)
        ...
        if self.vps_timing_info_present_flag:
            ...
            #self.vps_num_units_in_tick = s.read('uint:32')
            #self.vps_time_scale = s.read('uint:32')
            ...
            for i in range(self.vps_num_hrd_parameters):
                ...
                if i > 0:
                    self.cprms_present_flag.append(s.read('uint:1'))
                    self.hrdp = hrd_parameters(self.cprms_present_flag[i], self.vps_max_sub_layers_minus1)
...
def read_nal_unit(s, i, NumBytesInNalUnit):
    ...
    if nal_unit_type == NalUnitType.NAL_UNIT_CODED_SLICE_TRAIL_N or \
       nal_unit_type == NalUnitType.NAL_UNIT_CODED_SLICE_TRAIL_R:
        ...
        # Video parameter set
        video_parameter_set_rbsp(s).show(s)
        ...
        pic_parameter_set_rbsp(s).show(s)

def main():
    """
    """
    F = 'Lenel_video_LNR_Format.lnr'
    ...
    for i, n in zip(nals, size):
        print()
        print("!! Found NAL in offset {0:d} ({0:#x})".format((i+24)//8))
        #print((i+24)//8,n//8)

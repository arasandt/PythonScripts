...
class preprocesses:
    ...
    def collect_data(self):
        ...
        dataset = facenet.get_dataset(self.input_datadir) # gets list of classes for each label. Each class will contain class name and path to all its images.
        with tf.Graph().as_default():
            gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.5)
            sess = tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, log_device_placement=False))
            with sess.as_default():
                pnet, rnet, onet = detect_face.create_mtcnn(sess, './model')  # npy is numpy array file. Not sure what it contains.
                    ...
                    if not os.path.exists(output_filename):
                        try:
                            ...
                            if nrof_faces > 0:
                                ...
                                scaled_temp = misc.imresize(cropped_temp, (image_size, image_size), interp='bilinear') # there should be no photos which cuts the face, else this will fail

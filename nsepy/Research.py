...
def load_index_data(days_window):
    ...
    df.drop(['Turnover (Rs. Cr)','Shares Traded'], axis=1, inplace=True)
    return df
...
#    expdt_df = expdt_df[expdt_df['ExpiryDate'] <= index_df['Date'].max()]
    ...
    index_df['AllMax'] = index_df[['Open', 'High', 'Low', 'Close']].max(axis=1)
    index_df['AllMin'] = index_df[['Open', 'High', 'Low', 'Close']].min(axis=1)
    index_df['AllDiff'] = index_df['AllMax'] - index_df['AllMin']

    index_df['OpenMax'] = index_df[['Open', 'Close']].max(axis=1)
    index_df['OpenMin'] = index_df[['Open', 'Close']].min(axis=1)
    index_df['OpenDiff'] = index_df['OpenMax'] - index_df['OpenMin']

    
    vol_df = index_df[['Date','AllDiff','OpenDiff']]
    
    threshold = 75
    
    vol_df_ge = vol_df[vol_df['AllDiff'] >= threshold]
    vol_df_lt = vol_df[vol_df['AllDiff'] < threshold]
    
    #print(vol_df_ge100.head(),len(vol_df_ge100),len(vol_df_ge100) // len(vol_df) )
    #print(vol_df_lt100.head(),len(vol_df_lt100), len(vol_df_ge100) // len(vol_df))
    
    print('All  >= {0} - {1} count, {2}%'.format(str(threshold),len(vol_df_ge),len(vol_df_ge) * 100 // len(vol_df)))
    print('All  <  {0} - {1} count, {2}%'.format(str(threshold),len(vol_df_lt),len(vol_df_lt) * 100 // len(vol_df)))
    

    vol_df_ge = vol_df[vol_df['OpenDiff'] >= threshold]
    vol_df_lt = vol_df[vol_df['OpenDiff'] < threshold]

    print('Open >= {0} - {1} count, {2}%'.format(str(threshold),len(vol_df_ge),len(vol_df_ge) * 100 // len(vol_df)))
    print('Open <  {0} - {1} count, {2}%'.format(str(threshold),len(vol_df_lt),len(vol_df_lt) * 100 // len(vol_df)))

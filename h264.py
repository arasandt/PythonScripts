...
for k in list(runBeforeMapping.keys()):
  runBeforeMapping[k] = makeAutomat( runBeforeMapping[k] )
for k in list(totalZerosMappingDC.keys()):
  totalZerosMappingDC[k] = makeAutomat( totalZerosMappingDC[k] )
for k in list(totalZerosMapping.keys()):
...
class BitStream:
  ...
  def bits( self, howMany, info=None ):
    ...
    for i in range(howMany):
      ret = ret*2 + next(self.gen)
    self.index += howMany
    return ret

  def golomb( self, info=None ):
    zeros = 0
    while next(self.gen) == 0:
      zeros += 1
    ret = 0
    for i in range(zeros):
      ret = 2*ret + next(self.gen)
...
def removeEscape( buf ):
  ...
  print(type(buf))
  return buf.replace(b"\x00\x00\x03",b"\x00\x00")


def parseISlice( bs ):
#  assert ord(f.read(1)) == 0xE0 # set012 flags and reserved zero 5bits
  print([hex(bs.alignedByte()) for i in range(5)])
...
def parsePPS( bs ):
  pps = [bs.alignedByte() for i in range(5)]
...
#      payloadSize = struct.unpack_from("I", f.read(4))[0]
      ...
      for i in range(headerSize-12+1):
        c = bs.alignedByte()
    header = header[1:] + [c]
  fout.close()
...
if __name__ == "__main__":
  ...
  if '-f' == sys.argv[1]:
    pass #print functionalTest( path = sys.argv[2] )
  else:
